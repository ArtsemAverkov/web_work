...
public class ProductSortingController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<List<ModelProduct>> allProduct = modelProductService.readAllProduct();
        List<List<ModelProduct>> collect = allProduct.stream()
                .sorted()
                .collect(Collectors.toList());

        logger.info("ProductSortingController :" + collect);
            req.setAttribute("products", collect);
